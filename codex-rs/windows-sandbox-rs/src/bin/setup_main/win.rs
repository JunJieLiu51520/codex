...
fn lock_sandbox_bin_dir(payload: &Payload, sandbox_group_sid: &[u8], log: &mut File) -> Result<()> {
    lock_sandbox_dir(
...
fn run_provision_only(payload: &Payload, log: &mut File, sbx_dir: &Path) -> Result<()> {
